    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `${baseUrl}/api/inmuebles?populate=*&filters[categoria][nombreCategoria][$eq]=En Anticretico&filters[active][$eq]=true&pagination[page]=${page}&pagination[pageSize]=${ITEMS_PER_PAGE}`
        );
        const json = await res.json();
        setInmuebles(json.data || []);
        setTotalItems(json.meta?.pagination?.total || 0);
      } catch (error) {
        console.error('Error al cargar inmuebles en anticrético:', error);
      }
    };
      ...
      <div className="relative h-[60vh] w-full bg-gradient-to-br from-[#001E6C] via-[#0038a2] to-[#0057b7] flex items-center justify-center text-center px-4">
        ...
        >
          ...
          <p className="mt-4 text-white text-lg drop-shadow-md max-w-2xl mx-auto">
            Encuentra alternativas de inversión seguras y convenientes en Tarija. Departamentos, casas y más en modalidad anticrético.
          </p>
        </motion.div>
      </div>
...
export default AnticreticoPage;
