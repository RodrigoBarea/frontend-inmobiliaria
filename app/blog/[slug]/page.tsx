...
const PaginaDetalleBlog = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${baseUrl}/api/blogs?populate=portada,agente.fotoPrincipal&filters[slug][$eq]=${slug}`);
  ...
  useEffect(() => {
    const fetchOtros = async () => {
      try {
        const res = await fetch(`${baseUrl}/api/blogs?populate=portada&filters[slug][$ne]=${slug}&filters[active][$eq]=true&pagination[limit]=3`);
        const json = await res.json();
  ...
  const imageUrl = portada?.data?.attributes?.url;
  ...
  return (
    <>
      <section className="relative h-[70vh] md:h-[80vh] w-full bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${baseUrl}${imageUrl})` }}>
      ...
      <section className="relative z-10 -mt-10 bg-white rounded-t-3xl shadow-xl pt-12 pb-16 px-6 max-w-4xl mx-auto space-y-6">
        ...
        {contenido.map((block, i) => {
          if (block.type === 'heading') {
            return <h2 key={i} className="text-2xl font-bold text-[#001E6C] mt-6">{block.children.map((c: any) => c.text).join(' ')}</h2>;
          }
          if (block.type === 'paragraph') {
            return <p key={i} className="text-gray-700 text-lg leading-relaxed">{block.children.map((c: any) => c.text).join(' ')}</p>;
          }
          return null;
        })}
        ...
        {agente?.data && (
          <div className="mt-12 border-t pt-6 flex items-center gap-4">
            {agente.data.attributes.fotoPrincipal?.data?.attributes?.url && (
              <Image
                src={`${baseUrl}${agente.data.attributes.fotoPrincipal.data.attributes.url}`}
        ...
        {otrosBlogs.length > 0 && (
          <aside className="mt-16 border-t pt-10">
            <div className="flex justify-between items-center mb-6">
              ...
              <Link href="/blog/page/1" className="text-sm text-[#001E6C] font-semibold hover:underline">
                Ver todos
              </Link>
            </div>
            <div className="grid md:grid-cols-3 gap-6">
              {otrosBlogs.map((b) => {
                const url = b.attributes.portada?.data?.attributes?.url;
                const fecha = new Date(b.attributes.createdAt).toLocaleDateString('es-ES', {
                  year: 'numeric', month: 'short', day: 'numeric'
                });

                return (
                  <Link href={`/blog/${b.attributes.slug}`} key={b.id} className="group border rounded-xl overflow-hidden hover:shadow-lg transition">
                    <div className="relative h-[160px] w-full">
                      <Image
                        src={`${baseUrl}${url}`}
                        alt={b.attributes.titulo}
                        fill
