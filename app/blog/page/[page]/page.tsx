        ...
        <div className="max-w-7xl mx-auto px-6 space-y-12">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {blogs.map(({ id, attributes }) => {
              ...
              const portadaUrl = portada?.data?.attributes?.formats?.large?.url || portada?.data?.attributes?.url || '';
              const previewText = contenido?.[0]?.children?.map((c: any) => c.text).join(' ').slice(0, 100);
          ...
          <div className="flex justify-center items-center gap-2 pt-10">
            {page <= 1 ? (
              <button disabled className="px-3 py-2 rounded-full bg-gray-200 text-gray-400 cursor-not-allowed">
                <ChevronLeft className="w-5 h-5" />
              </button>
            ) : (
              <Link href={`/blog/page/${page - 1}`} className="px-3 py-2 rounded-full bg-gray-100 text-gray-700 hover:bg-gray-200">
                <ChevronLeft className="w-5 h-5" />
              </Link>
            )}
            ...
            {Array.from({ length: totalPages }, (_, i) => (
              <Link
                ...
                className={`w-10 h-10 flex items-center justify-center rounded-full text-sm font-semibold ${
                  i + 1 === page ? 'bg-[#1c39bb] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                {i + 1}
              </Link>
