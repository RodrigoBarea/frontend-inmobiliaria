    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `${baseUrl}/api/inmuebles?populate=*&filters[categoria][nombreCategoria][$eq]=En alquiler&filters[active][$eq]=true&pagination[page]=${page}&pagination[pageSize]=${ITEMS_PER_PAGE}`
        );
        const json = await res.json();
        setInmuebles(json.data || []);
        setTotalItems(json.meta?.pagination?.total || 0);
      } catch (error) {
        console.error('Error al cargar inmuebles en alquiler:', error);
      }
    };
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {inmuebles.map(({ id, attributes }) => {
              ...
              const nombreCategoria =
                categoria?.data?.attributes?.nombreCategoria || 'ALQUILER';
