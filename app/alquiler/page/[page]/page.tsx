...
const AlquilerPage = () => {
  ...
  const fetchTiposUnicos = async () => {
    try {
      const res = await fetch(
        `${baseUrl}/api/inmuebles?fields[0]=tipo&filters[categoria][nombreCategoria][$eq]=En alquiler&pagination[pageSize]=100`
      );
      ...
      console.log('Tipos de inmuebles cargados:', tiposUnicos); // Para depuración
    } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(
          `${baseUrl}/api/inmuebles?populate=*&filters[categoria][nombreCategoria][$eq]=En alquiler&filters[active][$eq]=true${filterQuery}&pagination[page]=${page}&pagination[pageSize]=${ITEMS_PER_PAGE}`
        );
  ...
  const handleFilterChange = async (tipo: string) => {
    ...
    await router.replace(`/alquiler/page/1?${params.toString()}`, {
      scroll: false
    });
  ...
  const clearFilters = async () => {
    await router.replace('/alquiler/page/1', {
      scroll: false
    });
  ...
  const getPaginatedUrl = (pageNumber: number) => {
    ...
    return `/alquiler/page/${pageNumber}?${params.toString()}`;
  };
